...
set -e
$MYSQL  -h$SERVER -P$DBPORT -u $USER -p$PASS -e "DROP DATABASE IF EXISTS $DATABASE"
$MYSQL  -h$SERVER -P$DBPORT -u $USER -p$PASS -e "CREATE DATABASE $DATABASE"
$MYSQL  -h$SERVER -P$DBPORT -u $USER -p$PASS $DATABASE < $TABLESQL
#$MYSQL  -h$SERVER -P$DBPORT -u $USER -p$PASS $DATABASE < $CONSTRAINTSQL

echo 'Loading item ...'
$TPCCLOAD $SERVER:$DBPORT $DATABASE $USER $PASS $WAREHOUSE 1 1 $WAREHOUSE > /dev/null
